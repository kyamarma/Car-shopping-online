...
$sql_1 = "SELECT claim_order.claim_id,claim_order.fName,claim_order.lName, claim_order.email, claim_order.contact, claim_order.Zip_code, claim_order.nid, claim_order.nationality, product.price,product.photo 
FROM claim_order 
JOIN product on product.model=claim_order.model_Name;";
if ($conn->query($sql_1)) {
  $result = $conn->query($sql_1);
}


$sql = "select * from claim_order";
if ($conn->query($sql)) {
  $result = $conn->query($sql);
}

if (isset($_GET['claim_id'])) {
  $sql = "delete from claim_order where claim_id=" . $_GET['claim_id'];
  if ($conn->query($sql)) {
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              <tr>
                ...
                <td><?= $row['Zip_code']; ?></td>
                <td>
                  <img src="<?= $row['nid']; ?>" width="100px" height="120px">
                </td>
                ...
                <td>
                  <a href="claim_order_id?claim_id=<?= $row['claim_id']; ?>">Edit</a>
                  <a onclick="return confirm('Are you sure?')"
                    href="order_list.php?claim_id=<?= $row['claim_id']; ?>">Delete</a>
                </td>
              </tr>
            <?php } ?>
          </tbody>
